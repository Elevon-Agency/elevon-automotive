...
export default async function AdminDashboardPage() {
  const [cars, featured, leads, available, sold, recentLeads, totalValue] =
    await Promise.all([
      prisma.car.count(),
  ...
  return (
    <div>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <Link
          href="/admin/cars/new"
          className="inline-flex h-11 items-center justify-center bg-primary px-6 text-sm font-medium text-primary-foreground"
        >
          Add car
        </Link>
      </div>
        ...
        <div className="border border-border p-6">
          ...
          <div className="mt-6 space-y-4 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span>{Math.max(cars - available - sold, 0)}</span>
            </div>
          </div>
        </div>
        <div className="border border-border p-6">
